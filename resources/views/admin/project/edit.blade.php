        ...
        <div class="card-body">
            <form action="{{ route('admin.project.update', $project) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="link" class="form-control @error('link') is-invalid @enderror" value="{{ old('link', $project->link) }}" required>
                    @error('name')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="date" name="date" class="form-control @error('date') is-invalid @enderror" value="{{ old('date', $project->date) }}" required>
                    @error('issued_at')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
